...
var tinyFilter_bg = {
	...
	generateSubscription : function(){
		var dataURI = "data:text/plain;charset=utf-8,%23 TINYFILTER EXTENSION%0A%23 LIST SUBSCRIPTION%0A%23 DATE: " +
			...
			"%0A%0A%23 Bloquear páginas com base nos seguintes critérios" +
	...
	loadSubscription : function(url){
		...
		for(var i = 0, len = file.length; i < len; i++){
			...
			if(!legit){
				alert("O arquivo de assinatura não é válido.");
				return false;
			}
	...
	fixSettings : function(){
		if(localStorage.getItem('content_filter') == null){
			...
			var content_filter = {
				enabled : true,
				advanced : {
					warning : "Esta página não está disponível devido a restrições do administrador.",
					stop_all : false,
					reason : true,
					redirect : ""
				},
...
var popup = {
	...
	*/
	filter: function(action, tab){
		...
		if(action === 0){
			var response = window.confirm("Bloquear \""+domain+"\"?");
		}else{
			var response = window.confirm("Confiar em \"" + domain + "\"?");
		}
		if(response){
			...
			var input = window.prompt("Por favor, insira sua senha.", "");
				if(Crypto.SHA256(input) !== gs.password.hash){
					alert("Ação negada: a senha está incorreta.");
					return 0;
				}
			}
